...
def ask_problem_projection(chat, problem_text, problem_reformation, response_save_path):
    chat_prompt = f"{problem_reformation} \n {problem_text}"
    return get_and_save_response(chat, chat_prompt, response_save_path)

def get_problem_projection_pipeline(chat,problem, save_suffix, force_search = False):
    print("\t1. projecting problem")
    simplification_response_save_path = os.path.join("Responses", "Simplification", save_suffix)
    ask_problem_projection(chat, problem, PROBLEM_REFORMATION_PREFIX, simplification_response_save_path)
    ...
    projection_answer = get_and_save_response(chat, PROJECTION_ANSWERING_PREFIX,  projection_answer_path, force_search=force_search)

    return projection_answer, chat

def get_problem_reprojection(chat, problem_projection, save_suffix, force_search = False):
    prompt  = f"{PROBLEM_REPROJECTION_PREFIX} \n answer: {problem_projection}"
...
def full_pipeline(problem_d, id_count,provider, model_name, api_key ="", grade_projected = True,
    ...
    if grade_projected:
        ...
        problem_projection = get_problem_projection_pipeline(chat, problem, save_suffix)
        ...
        grade_answer_high_reasoning(problem,problem_projection, rubric, projection_grade_save_path, projection_grade_full_chat_path)
        if not keep_chat:
        ...
        if grade_reprojected:
            if keep_chat:
                ...
                reprojected_answer = get_problem_reprojection(chat, problem_projection, reprojection_path, force_search=force_search)
                projection_histories.append(chat.get_history())
                # saving history:
                ...
                grade_answer_high_reasoning(problem, reprojected_answer, rubric, reprojection_grade_save_path,reprojection_grade_full_chat_path)
    ...
    if grade_regular:
        ...
        grade_answer_high_reasoning(problem, default_answer, rubric, default_grade_save_path, default_grade_full_chat_path)
...
def grade_answer_high_reasoning(problem, answer, rubric, response_save_path, history_path, grade_from_template = True):
    if grade_from_template:
        prompt = GRADING_TEMPLATE.format(problem=problem, rubric = rubric, answer=  answer)
    else:
        prompt = f"{GRADING_ANSWER_PREFIX} {answer} \n Rubric:\n {rubric}\n"
